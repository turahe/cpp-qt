...
\begin{itemize}
\item Konsep dasar fungsi
\item Cara mendefinisikan fungsi
\item Parameter dan return value
\item Jenis-jenis fungsi
\end{itemize}

\minitoc

\section{Fungsi (Function)}\label{sec-fungsi}

\subsection{Apa itu Fungsi?}
...
\subsection{Karakteristik Fungsi}

\begin{itemize}
\item \textbf{Bernama} - setiap fungsi memiliki nama unik
\item \textbf{Dapat dipanggil} - fungsi bisa digunakan berulang kali
\item \textbf{Mengembalikan nilai} - fungsi dapat memberikan hasil
\item \textbf{Manipulasi data} - fungsi dapat memproses data
\end{itemize}

\subsection{Fungsi main()}

\begin{itemize}
\item Setiap program C++ memiliki fungsi \texttt{main()}
\item Fungsi ini dipanggil otomatis saat program dijalankan
\item Fungsi lain hanya bekerja ketika dipanggil
\item Fungsi \texttt{main()} dapat diakses dari mana saja dalam program
\end{itemize}

\subsection{Cara Kerja Fungsi}

\begin{itemize}
\item Ketika nama fungsi dipanggil, eksekusi program dialihkan ke tubuh fungsi
\item Setelah selesai (ditandai dengan \texttt{return} atau kurung kurawal tutup), program kembali ke baris berikutnya
\item Peristiwa ini disebut \textbf{pemanggilan fungsi}
\end{itemize}

\subsubsection{Ilustrasi Pemanggilan Fungsi}

Ilustrasi pemanggilan fungsi seperti gambar \ref{gambar4-1}.

\begin{figure}[htbp]
\centering
\includegraphics[width=8cm]{Capture4-1}
\label{gambar4-1}
\caption{Ilustrasi penanganan fungsi}
\end{figure}

\subsection{Karakteristik Fungsi yang Baik}

\begin{itemize}
\item \textbf{Spesifik} - mengerjakan satu pekerjaan tertentu
\item \textbf{Mudah dipahami} - logika yang jelas
\item \textbf{Nama yang jelas} - mudah dikenali berdasarkan namanya
\item \textbf{Terpecah} - pekerjaan kompleks dipecah menjadi beberapa fungsi
\end{itemize}

\subsection{Jenis-jenis Fungsi}

\begin{itemize}
\item \textbf{Fungsi Standard (Built-in)} - sudah tersedia dalam kompiler
\item \textbf{Fungsi User-defined} - dibuat sendiri oleh programmer
\end{itemize}

\section{Konsep Dasar Fungsi}\label{konsep-dasar-fungsi}

\subsection{Fungsi vs Prosedur}

Fungsi mirip dengan prosedur (dalam bahasa Pascal), keduanya disebut \textbf{Subrutin}.

\subsubsection{Perbedaan Fungsi dan Prosedur}

\begin{itemize}
\item \textbf{Fungsi} - selalu mengembalikan nilai setelah dipanggil
\item \textbf{Prosedur} - tidak mengembalikan nilai
\end{itemize}

\subsection{Mengapa Perlu Subrutin?}

\begin{itemize}
\item \textbf{Program besar} - lebih baik jika tugas tertentu dilakukan oleh subrutin tertentu
\item \textbf{Mudah dibaca} - program menjadi lebih terstruktur
\item \textbf{Mudah dipelihara} - perubahan hanya perlu dilakukan di satu tempat
\end{itemize}
\begin{quotation}
{\LARGE \ding{45}} \textbf{Catatan Penting}

Fungsi bisa dikatakan sebagai bentuk lain dari instruksi yang dapat memberikan sebuah nilai apabila diberi masukan yang dibutuhkan. Masukan tersebut dikenal dengan istilah \textbf{Parameter}.
\end{quotation}


\subsection{Fungsi dalam Program C++}

Fungsi adalah elemen utama dari program C++. Program C++ dibentuk dari kumpulan fungsi:
\begin{itemize}
\item Fungsi utama \texttt{main()}
\item Fungsi pustaka (standar)
\item Fungsi yang dibuat sendiri (UDF = User Defined Function)
\end{itemize}

\subsection{Alasan Menggunakan Fungsi}

\begin{enumerate}
\item \textbf{Struktur yang jelas} - memisahkan langkah-langkah detail ke fungsi-fungsi, membuat \texttt{main()} lebih pendek dan mudah dimengerti
\item \textbf{Menghindari pengulangan} - program yang sama tidak perlu ditulis berulang-ulang, cukup memanggil fungsi
\end{enumerate}

\subsection{Tugas Fungsi}
...
\item Penyeleksian
\item Perhitungan
\item Dan tugas-tugas lainnya
\end{itemize}

\section{Mendefinisikan Fungsi}\label{mendefinisikan-fungsi}

\subsection{Komponen Fungsi}

Fungsi terdiri dari dua komponen utama:
\begin{itemize}
\item \textbf{Definisi fungsi} - tipe fungsi, nama fungsi, dan parameter
\item \textbf{Tubuh fungsi} - statemen-statemen yang melakukan tugas
\end{itemize}
...
\lstinputlisting[language=c++]{../code/04-fuction-code-1.c++}
...
\lstinputlisting[language=c++]{../code/04-fuction-code-2.c++}
...
\lstinputlisting[language=c++]{../code/04-fuction-code-4.c++}
