...
\begin{itemize}
\item Pengenalan inheritance dan konsepnya
\item Cara membuat kelas turunan
\item Jenis akses dalam inheritance
\item Code reuse dan fungsionalitas
\end{itemize}
...
\subsection{Apa itu Inheritance?}
...
\begin{itemize}
\item \textbf{Base Class} - kelas yang sudah ada (kelas dasar)
\item \textbf{Derived Class} - kelas baru yang dibuat dari kelas dasar
\item \textbf{Code Reuse} - menggunakan kode yang sudah ada
\item \textbf{Extensibility} - menambah fungsionalitas baru
\end{itemize}

\subsection{Cara Kerja Inheritance}

\begin{itemize}
\item Kelas yang sudah ada dapat dikembangkan menjadi kelas baru
\item Semua anggota kelas dasar (yang tidak private) diwarisi oleh kelas turunan
\item Pemrogram tinggal menambahkan anggota baru untuk fungsionalitas tambahan
\item Inheritance akan dibahas bersama dengan polimorfisme di bab berikutnya
\end{itemize}

\section{📝 Penulisan Inheritance}\label{penulisan-penurunan}

\subsection{Sintaks Inheritance}
...
\subsubsection{Bentuk Umum}

\lstinputlisting[language=c++]{../code/07-turunan-code-1.c++}

\subsubsection{Contoh Inheritance}

Sebagai contoh, jika akan dibuat kelas turunan Silinder dari kelas dasar Lingkaran:

\lstinputlisting[language=c++]{../code/07-turunan-code-2.c++}

\subsection{Penting untuk Diingat}

\begin{itemize}
\item Jenis akses \textbf{public} akan dibahas lebih detail nanti
\item Kelas dasar harus sudah dideklarasikan sebelum kelas turunan
\item Jika tidak, akan muncul pesan kesalahan kompiler
\end{itemize}
...
\begin{itemize}
\item \textbf{Kelas Lingkaran} - memiliki variabel anggota \texttt{radius} bertipe \texttt{float} dan fungsi \texttt{getLuas()} yang mengembalikan nilai \texttt{float}

\item \textbf{Kelas Silinder} - turunan dari kelas \texttt{Lingkaran}, ditulis: \texttt{class Silinder : Lingkaran}. Kelas ini mewarisi anggota kelas \texttt{Lingkaran} yang tidak private

\item \textbf{Inheritance} - kelas \texttt{Silinder} mewarisi variabel \texttt{radius} dan fungsi \texttt{getLuas()} dari kelas \texttt{Lingkaran}

\item \textbf{Penambahan Fungsionalitas} - kelas \texttt{Silinder} menambahkan variabel \texttt{tinggi} dan fungsi \texttt{getVolume()}

\item \textbf{Code Reuse} - fungsi \texttt{getVolume()} menggunakan \texttt{getLuas()} yang diwarisi: \texttt{return getLuas() * tinggi}

\item \textbf{Hasil Eksekusi} - program menghitung volume silinder dengan rumus \emph{(3.14 * 50 * 50) * 125} = 981250
\end{itemize}
...
\lstinputlisting[language=c++]{../code/07-turunan-code-5.c++}
...
\lstinputlisting[language=c++]{../code/07-turunan-code-6.c++}
...
\lstinputlisting[language=c++]{../code/07-turunan-code-7.c++}
...
\lstinputlisting[language=c++]{../code/07-turunan-code-8.c++}
...
\lstinputlisting[language=c++]{../code/07-turunan-code-9.c++}
...
\lstinputlisting[language=c++]{../code/07-turunan-code-11.c++}
...
\lstinputlisting[language=c++]{../code/07-turunan-code-12.c++}
...
\lstinputlisting[language=c++]{../code/07-turunan-code-14.c++}
\item
...
\lstinputlisting[language=c++]{../code/07-turunan-code-15.c++}

Maka tipe data variabel tersebut bisa di-``down-casted'' menjadi tipe
data kelas turunan dengan cara:

\lstinputlisting[language=c++]{../code/07-turunan-code-16.c++}
...
\lstinputlisting[language=c++, caption=Memakai static\_cast, label=contoh7-14]{../code/07-turunan-contoh7-14.c++}
