...
\begin{itemize}
\item Pengenalan tipe data dan identifier
\item Jenis-jenis tipe data dalam C++
\item Variabel dan konstanta
\item Operator dan ekspresi
\end{itemize}
...
\subsection{Apa itu Data?}

Segala sesuatu yang diproses oleh program adalah \textbf{data}. Data adalah elemen-elemen yang memiliki nilai atau besaran, misalnya:
\begin{itemize}
\item \textbf{Umur} - bisa berupa angka 42.5 (42½ tahun)
\item \textbf{Golongan} - bisa berupa karakter 'A'
\item \textbf{Nama} - bisa berupa teks "John Doe"
\end{itemize}
...
\begin{itemize}
\item \textbf{Tipe Data} - menentukan jenis dan ukuran data
\item \textbf{Variabel} - tempat menyimpan data yang bisa berubah
\item \textbf{Konstanta} - tempat menyimpan data yang tetap
\end{itemize}

\subsection{Apa itu Identifier?}
...
\subsubsection{Mengapa Perlu Identifier?}

\begin{itemize}
\item Alamat memori sebenarnya berupa angka heksadesimal yang sulit diingat
\item C++ menggunakan identifier berupa huruf atau kata sebagai label
\item Dengan identifier, kita tidak perlu mengetahui alamat memori yang sebenarnya
\item Identifier membuat program lebih mudah dibaca dan dipahami
\end{itemize}

\section{📊 Tipe Data Bahasa C++}\label{tipe-data-bahasa-c}

\subsection{Apa itu Tipe Data?}

Data yang dapat dikelola program bisa bermacam-macam:
\begin{itemize}
\item Bilangan bulat (integer)
\item Bilangan desimal (floating point)
\item Karakter (character)
\item Dan lainnya
\end{itemize}

\subsection{Mengapa Perlu Tipe Data?}

Ketika menyimpan nilai di memori, diperlukan 2 hal:
\begin{itemize}
\item \textbf{Identifier} - nama untuk mengakses lokasi memori
\item \textbf{Tipe Data} - menentukan jenis dan ukuran memori yang dialokasikan
\end{itemize}

\subsection{Pentingnya Tipe Data}

\begin{itemize}
\item Sekali identifier dialokasikan dengan tipe data tertentu, ukuran ruang tidak bisa diubah
\item Tipe data menentukan operasi apa yang bisa dilakukan
\item Tipe data mempengaruhi efisiensi penggunaan memori
\end{itemize}
...
\subsection{Apa itu Variabel dan Konstanta?}

Nilai yang tersimpan di memori dan dikenal melalui identifier terdiri dari variabel dan konstanta.

\subsection{Perbedaan Variabel dan Konstanta}

\begin{itemize}
\item \textbf{Variabel} - nilainya dapat diubah-ubah saat program dieksekusi
\item \textbf{Konstanta} - nilainya tidak dapat diubah (tetap/konstan)
\end{itemize}

\subsection{Analoginya}

\begin{itemize}
\item \textbf{Variabel} seperti kotak yang bisa diisi dengan barang berbeda
\item \textbf{Konstanta} seperti kotak yang hanya bisa diisi dengan satu barang tertentu
\end{itemize}
...
\lstinputlisting[language=c++]{../code/02-tipedata-code-3.c++}
...
\lstinputlisting[language=c++]{../code/02-tipedata-code-4.c++}
...
\lstinputlisting[language=c++]{../code/02-tipedata-code-5.c++}
...
\lstinputlisting[language=c++]{../code/02-tipedata-code-6.c++}
...
\lstinputlisting[language=c++]{../code/02-tipedata-code-7.c++}
...
\lstinputlisting[language=c++]{../code/02-tipedata-code-8.c++}
