#!/usr/bin/env python3
"""
Enhanced LaTeX to Markdown Converter
Converts LaTeX files to Markdown format with improved formatting
"""
...
class EnhancedLatexToMarkdown:
    ...
    def convert_latex_commands(self, content):
        ...
        # Convert code listings (enhanced)
        content = self.convert_code_listings_enhanced(content)
        
        # Convert images
        content = self.convert_images(content)
        
        # Convert tables
        content = self.convert_tables_enhanced(content)
        ...
        content = self.convert_center_environment(content)
        
        # Convert figure environment
        content = self.convert_figure_environment(content)
        
        # Convert table environment
        content = self.convert_table_environment(content)
        
        # Convert custom commands
        content = self.convert_custom_commands(content)
    ...
    def convert_code_listings_enhanced(self, content):
        """Convert LaTeX code listings to Markdown code blocks with better handling"""
        
        # Handle lstlisting with caption and lstinputlisting
        content = re.sub(
        ...
        # Handle lstinputlisting with caption in separate lstlisting
        content = re.sub(
            r'\\begin\{lstlisting\}\[language=c\+\+, caption=([^\]]+)\]\s*\\lstinputlisting\[language=c\+\+\]\{([^}]+)\}\s*\\end\{lstlisting\}',
            r'```cpp\n// \1\n// File: \2\n```',
            content
        ...
        # Handle lstlisting environments with content
        content = re.sub(
        ...
        # Handle simple lstlisting without options
        content = re.sub(
            r'\\begin\{lstlisting\}(.*?)\\end\{lstlisting\}',
            r'```cpp\1```',
            content,
    ...
    def convert_tables_enhanced(self, content):
        """Convert LaTeX tables to Markdown tables with better handling"""
        
        # Convert simple tabular environments
        content = re.sub(
            r'\\begin\{tabular\}\{([^}]+)\}(.*?)\\end\{tabular\}',
            self.convert_tabular_to_markdown,
            content,
            flags=re.DOTALL
        )
        
        # Remove complex table environments for now
        content = re.sub(
            r'\\begin\{table\}.*?\\end\{table\}',
            r'[Table - convert manually]',
            content,
            flags=re.DOTALL
        )
    ...
    def convert_tabular_to_markdown(self, match):
        """Convert tabular environment to Markdown table"""
        columns = match.group(1)
        content = match.group(2)
        
        # Count columns
        col_count = len(columns.replace('|', ''))
        ...
        for i, row in enumerate(rows):
            if row.strip():
                ...
                # Convert LaTeX commands in cells
                cells = [self.convert_cell_content(cell) for cell in cells]
    ...
    def convert_cell_content(self, cell):
        """Convert LaTeX commands in table cells"""
    ...
    def convert_center_environment(self, content):
        """Convert center environment to Markdown center"""
        
        content = re.sub(r'\\begin\{center\}(.*?)\\end\{center\}', r'<div align="center">\1</div>', content, flags=re.DOTALL)
        
        return content
    
    def convert_figure_environment(self, content):
        """Convert figure environment to Markdown"""
        
        content = re.sub(
            r'\\begin\{figure\}.*?\\includegraphics.*?\{([^}]+)\}.*?\\end\{figure\}',
            r'![Figure](\1)',
            content,
            flags=re.DOTALL
        )
    ...
    def convert_table_environment(self, content):
        """Convert table environment to Markdown"""
    ...
    def convert_custom_commands(self, content):
        """Convert custom LaTeX commands"""
        
        # Convert common custom commands
        content = re.sub(r'\\emph\{([^}]+)\}', r'*\1*', content)
        content = re.sub(r'\\url\{([^}]+)\}', r'[\1](\1)', content)
        content = re.sub(r'\\email\{([^}]+)\}', r'[\1](mailto:\1)', content)
...
def main():
    converter = EnhancedLatexToMarkdown()
    
    print("🔄 Starting Enhanced LaTeX to Markdown conversion...")
    print("=" * 50)
    ...
    print("=" * 50)
    print("✅ Enhanced LaTeX to Markdown sync completed!")
